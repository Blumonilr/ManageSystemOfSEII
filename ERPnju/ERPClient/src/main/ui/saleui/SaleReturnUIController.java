...
public class SaleReturnUIController {
	...
	public void onSubmit(){
		if(this.checkComplete()){
			...
			if(res!=null){
				info="创建成功！";
				this.onClear();
				ClientRunner.getLogger().record("创建销售退货单  id: "+res.getId());
			}
	...
	public void onSaveDraft(){
		...
		try {
			...
			res=saleblservice.makeSaleReturnReceiptDraft(receipt, currentUserId);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(res!=null){
			info="创建成功！";
			ClientRunner.getLogger().record("创建销售退货单草稿 id: "+res.getId());
			this.onClear();
		}
		Alert alert=new Alert(AlertType.INFORMATION);
		alert.setTitle("提示");
