...
import main.ui.saleui.SaleReturnUIController.TableNode;
...
public class EditSaleReturnReceiptUIController {
	...
	public void onInit(){
		System.out.println("SaleReturnInit");
	...
	public void onSubmit(){
		if(this.checkComplete()){
			...
			SaleReturnReceiptVO receipt=this.getReceiptFromUI();
			SaleReturnReceiptVO res=null;
			try {
				saleblservice.deleteDraft(receipt.getId(),currentUserId);
				res=saleblservice.makeSaleReturnReceipt(receipt);
			...
			if(res!=null){
				info="创建成功！";
				ClientRunner.getLogger().record("通过草稿创建销售退货单  id: "+res.getId());
			}
	...
	public void onSaveDraft(){
		...
		try {
			...
			res=saleblservice.modifySaleReturnReceiptDraft(receipt, currentUserId);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(res!=null){
			info="保存成功！";
			ClientRunner.getLogger().record("修改销售退货单草稿  id: "+res.getId());
		}
		Alert alert=new Alert(AlertType.INFORMATION);
		alert.setTitle("提示");
		alert.setHeaderText(null);
	...
	public void onSearchStrategy(){
		SaleReturnReceiptVO receipt=this.getReceiptFromUI();//要传给strategy界面的
		if(receipt==null){
	...
	private SaleReturnReceiptVO getReceiptFromUI(){
		SaleReturnReceiptVO vo=null;
		...
		try{
			reductionOfPackage=Double.parseDouble(this.otherT.getText());
		}catch(Exception e){
		...
		vo=new SaleReturnReceiptVO(id, Calendar.getInstance(),null,creatorId,
