...
public class PurchaseReturnUIController {
	...
	public void onSubmit(){
		if(this.checkComlplete()){
			if(this.checkNum()){
				...
				if(res!=null){
					info="创建成功";
					ClientRunner.getLogger().record("提交销售退货单 id: "+res.getId());
				}
	...
	public void onSaveDraft(){
		...
		purchaseReturnReceiptDraft=new PurchaseReturnReceiptVO(null,Calendar.getInstance(),null,operatorId,clientId,stockId,note,itemList);
		
		PurchaseReturnReceiptVO res=null;
		try {
			res=purchaseblservice.makePurchaseReturnReceiptDraft(purchaseReturnReceiptDraft, currentUserId);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(res!=null){
			info="创建成功";
			ClientRunner.getLogger().record("创建销售退货单单据 id: "+res.getId());
		}
		Alert alert=new Alert(AlertType.INFORMATION);
		alert.setTitle("提示");
		alert.setHeaderText(null);
