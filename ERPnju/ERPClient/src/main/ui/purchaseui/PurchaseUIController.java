...
public class PurchaseUIController {
	...
	public void onSubmit(){
		if(this.checkComlplete()){
			...
			if(res!=null){
				info="创建成功！";
				ClientRunner.getLogger().record("提交进货单 id: "+res.getId());
			}
	...
	public void onSaveDraft(){
		...
		purchaseReceiptDraft=new PurchaseReceiptVO(null,Calendar.getInstance(),null,operatorId,clientId,stockId,note,itemList);
		PurchaseReceiptVO res=null;
		try {
			res=purchaseblservice.makePurchaseReceiptDraft(purchaseReceiptDraft, currentUserId);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(res!=null){
			info="创建成功！";
			ClientRunner.getLogger().record("创建进货单草稿 id: "+res.getId());
		}
		Alert alert=new Alert(AlertType.INFORMATION);
		alert.setTitle("提示");
		alert.setHeaderText(null);
	...
	private boolean checkComlplete(){
