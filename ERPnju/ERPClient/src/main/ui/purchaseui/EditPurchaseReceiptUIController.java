...
public class EditPurchaseReceiptUIController {
	...
	public void onSubmit(){
		if(this.checkComplete()){
			...
			purchaseReceipt=new PurchaseReceiptVO(null,Calendar.getInstance(),null,operatorId,clientId,stockId,note,itemList);
			PurchaseReceiptVO res=null;
			try {
				res=purchaseblservice.makePurchaseList(purchaseReceipt);
			...
			if(res!=null){
				info="创建成功！";
				ClientRunner.getLogger().record("通过草稿创建进货单  id: "+res.getId());
			}
	...
	public void onSaveDraft(){
		...
		PurchaseReceiptVO purchaseReceiptDraft=null;
		String clientId=this.clientId.getText();
		String stockId=this.stockId.getText();
		String operatorId=this.operatorId.getText();
		String note=this.note.getText();
		...
		purchaseReceiptDraft=new PurchaseReceiptVO(this.receiptId.getText(),Calendar.getInstance(),null,operatorId,clientId,stockId,note,itemList);
		PurchaseReceiptVO res=null;
		try {
			res=purchaseblservice.modifyPurchaseReceiptDraft(purchaseReceiptDraft, currentUserId);
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		if(res!=null){
			info="保存成功！";
			ClientRunner.getLogger().record("修改进货单草稿  id: "+res.getId());
		}
		Alert alert=new Alert(AlertType.INFORMATION);
		alert.setTitle("提示");
		alert.setHeaderText(null);
